-- Repair migration: ensure staff_points_rule exists.
-- V9__m6_points_rule.sql has a version (9) below flyway baseline-version (11),
-- so on any database that was baselined (baseline-on-migrate=true) it was skipped
-- and the table never got created -> /api/admin/hr/points/rule/page returned 503.
-- This forward migration re-creates it idempotently (no-op where V9 already ran).
