...
-- elder 补齐租户与审计字段
ALTER TABLE elder ADD COLUMN IF NOT EXISTS tenant_id BIGINT NOT NULL DEFAULT 0;
ALTER TABLE elder ADD COLUMN IF NOT EXISTS created_by BIGINT DEFAULT NULL;

UPDATE elder SET tenant_id = org_id WHERE tenant_id = 0;

CREATE INDEX IF NOT EXISTS idx_elder_tenant_id ON elder (tenant_id);

-- elder_bed_relation 补齐租户与审计字段
ALTER TABLE elder_bed_relation ADD COLUMN IF NOT EXISTS tenant_id BIGINT NOT NULL DEFAULT 0;
ALTER TABLE elder_bed_relation ADD COLUMN IF NOT EXISTS created_by BIGINT DEFAULT NULL;

UPDATE elder_bed_relation SET tenant_id = org_id WHERE tenant_id = 0;

CREATE INDEX IF NOT EXISTS idx_elder_bed_rel_tenant_id ON elder_bed_relation (tenant_id);
