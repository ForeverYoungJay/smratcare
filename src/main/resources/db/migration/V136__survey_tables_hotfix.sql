-- 热修：部分历史环境缺少问卷核心表，导致 survey_template 查询报错
...
CREATE TABLE IF NOT EXISTS survey_template (
  ...
  content TEXT DEFAULT NULL COMMENT '问卷内容说明',
...
SET @survey_template_content_exists := (
  SELECT COUNT(1)
...
SET @ddl_add_survey_template_content_hotfix := IF(
  @survey_template_content_exists = 0,
  'ALTER TABLE survey_template ADD COLUMN content TEXT DEFAULT NULL COMMENT ''问卷内容说明'' AFTER description',
  'SELECT 1'
);
PREPARE stmt_add_survey_template_content_hotfix FROM @ddl_add_survey_template_content_hotfix;
EXECUTE stmt_add_survey_template_content_hotfix;
DEALLOCATE PREPARE stmt_add_survey_template_content_hotfix;
