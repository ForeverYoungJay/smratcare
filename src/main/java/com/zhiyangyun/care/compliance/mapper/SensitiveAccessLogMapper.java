...
@Mapper
public interface SensitiveAccessLogMapper extends BaseMapper<SensitiveAccessLog> {

  /** 留存清理：按机构物理删除超期日志。 */
  @Delete("DELETE FROM compliance_sensitive_access_log WHERE org_id = #{orgId} AND create_time < #{before}")
  int deleteByOrgBefore(@Param("orgId") Long orgId, @Param("before") LocalDateTime before);

  /** 留存清理：按默认留存物理删除超期日志（排除已单独配置策略的机构）。 */
  @Delete("<script>DELETE FROM compliance_sensitive_access_log WHERE create_time &lt; #{before}"
