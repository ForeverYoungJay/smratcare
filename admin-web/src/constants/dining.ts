...
export function getDiningMealTypeLabel(value?: string) {
  return DINING_MEAL_TYPE_OPTIONS.find((item) => item.value === value)?.label || value || '-'
}

export function getDiningOrderStatusLabel(value?: string) {
  return DINING_ORDER_STATUS_OPTIONS.find((item) => item.value === value)?.label || value || '-'
}

export function getDiningEnableStatusLabel(value?: string) {
  return DINING_ENABLE_STATUS_OPTIONS.find((item) => item.value === value)?.label || value || '-'
}

export function getDiningDeliveryStatusLabel(value?: string) {
  return DINING_DELIVERY_STATUS_OPTIONS.find((item) => item.value === value)?.label || value || '-'
}

export function getDiningOrderStatusColor(value?: string) {
  if (value === DINING_STATUS.delivered) return 'green'
  if (value === DINING_STATUS.cancelled) return 'red'
  if (value === DINING_STATUS.delivering) return 'blue'
  return 'orange'
}

export function getDiningDeliveryStatusColor(value?: string) {
  if (value === DINING_STATUS.delivered) return 'green'
  if (value === DINING_STATUS.failed) return 'red'
  return 'orange'
}
