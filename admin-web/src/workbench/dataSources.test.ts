...
vi.mock('../api/care', () => ({ getTaskSummary: vi.fn() }))
...
describe('department workbench data source', () => {
  ...
  it.each([
    ...
    await loadDepartmentWorkbenchSnapshot(resolveWorkbenchProfile([role]), () => true)
  ...
  it('normalizes real nursing summary fields into shared metrics', async () => {
    ...
    const result = await loadDepartmentWorkbenchSnapshot(resolveWorkbenchProfile(['NURSING_EMPLOYEE']), () => true)
  ...
  it('distinguishes empty data from request failures', async () => {
    ...
    expect((await loadDepartmentWorkbenchSnapshot(resolveWorkbenchProfile(['NURSING_EMPLOYEE']), () => true)).status).toBe('empty')
    vi.mocked(getTaskSummary).mockRejectedValue(new Error('network down'))
    expect((await loadDepartmentWorkbenchSnapshot(resolveWorkbenchProfile(['NURSING_EMPLOYEE']), () => true)).status).toBe('error')
  })
})
