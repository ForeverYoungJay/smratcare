<template>
  <PageContainer title="员工餐费" subTitle="费用与报销 / 员工餐费">
...
import { getHrMealFeePage } from '../../api/hr'
...
async function fetchData() {
  loading.value = true
  try {
    const res: PageResult<HrExpenseItem> = await getHrMealFeePage({ ...query })
    rows.value = res.list || []
...
function onExportCsv() {
  exportCsv(mapByDict(rows.value as Record<string, any>[], HR_EXPENSE_EXPORT_FIELDS), '员工餐费-当前筛选')
}

function onExportExcel() {
  exportExcel(mapByDict(rows.value as Record<string, any>[], HR_EXPENSE_EXPORT_FIELDS), '员工餐费-当前筛选')
}
