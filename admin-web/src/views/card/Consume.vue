<template>
  <PageContainer title="消费记录" subTitle="一卡通消费扣费记录">
    ...
    <a-modal v-model:open="open" title="一卡通消费" @ok="submit" :confirm-loading="saving" width="560px">
      <a-form layout="vertical">
        ...
        <a-form-item label="备注">
          <a-input v-model:value="form.remark" placeholder="消费说明" />
        </a-form-item>
      </a-form>
    </a-modal>
  </PageContainer>
...
const columns = [
  ...
  { title: '消费金额', dataIndex: 'amount', key: 'amount', width: 120 },
...
async function fetchData() {
  loading.value = true
  try {
    const res: PageResult<CardTradeLog> = await getCardConsumePage({
      pageNo: query.pageNo,
