...
function buildSceneData() {
  ...
  props.buildings.forEach((buildingName, bIndex) => {
    ...
    bFloors.forEach((floorName, fIndex) => {
      ...
      fGroup.userData = { type: 'floor', building: buildingName, name: floorName }
      fGroup.position.y = fIndex * floorHeight
      ...
      rooms.forEach((roomItem, rIndex) => {
        const rGeo = new THREE.BoxGeometry(roomSize, 2, roomSize)
        ...
        roomMesh.position.set(rx, 1.4, rz)
        roomMesh.userData = { type: 'room', building: buildingName, floor: floorName, room: roomItem }
        
        // Add room label
        const rLabel = createLabel(roomItem.roomNo, 'room-label')
    ...
    const totalHeight = Math.max(bFloors.length * floorHeight, floorHeight)
        ...
        if (bName === selectedBuilding.value) {
          ...
          obj.children.forEach((c, idx) => {
            if (c.userData.type === 'floor') {
              ...
              gsap.to(c.position, { y: idx * 8, duration: 0.8, ease: "power2.out" })
              // Show floor labels, hide room labels
              c.children.forEach(cc => {
                if (cc.userData.type === 'room') {
                  const rl = cc.children.find(ccc => ccc instanceof CSS2DObject)
                  if (rl) rl.visible = false
                }
                if (cc instanceof CSS2DObject && cc.element.className.includes('floor-label')) cc.visible = true
              })
            }
          })
        } else {
          ...
          obj.children.forEach((c) => {
            if (c.userData.type === 'floor') {
              if (c.userData.name === selectedFloor.value) {
                ...
                // Show room labels, hide floor label
                c.children.forEach(cc => {
                  if (cc.userData.type === 'room') {
                    const rl = cc.children.find(ccc => ccc instanceof CSS2DObject)
...
function onPointerMove(event: PointerEvent) {
  ...
  if (hoveredObj && hoveredObj !== hits[0]?.object) {
    ...
    } else if (hoveredObj.userData.type === 'room') {
  ...
  if (hits.length > 0) {
    ...
    if (obj !== hoveredObj) {
      ...
      } else if (obj.userData.type === 'room') {
...
function onClick() {
  if (hoveredObj) {
    if (hoveredObj.userData.type === 'buildingHitbox') {
      ...
      emit('click-room', hoveredObj.userData.room)
    }
  }
}
