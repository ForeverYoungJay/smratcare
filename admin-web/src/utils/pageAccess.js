...
const LEGACY_PERMISSION_PATH_MAP = {
    '/oa/portal': '/workbench',
...
function toCanonicalPermissionPath(path) {
...
function shouldIncludeRoute(route, fullPath) {
...
function buildTree(routeList, basePath = '') {
    return (routeList || [])
        .map((route) => {
...
const pagePermissionTree = buildTree((layout?.children || []));
const pagePermissionFlat = [];
...
export function getPagePermissionTree() {
...
export function normalizePagePermissions(paths) {
...
export function parseRoutePermissionsJson(value) {
    if (!value)
        return [];
    try {
        const parsed = JSON.parse(value);
        return Array.isArray(parsed) ? normalizePagePermissions(parsed) : [];
    }
    catch {
        return [];
    }
}
export function serializeRoutePermissions(paths) {
    return JSON.stringify(normalizePagePermissions(paths));
}
export function shouldPersistExplicitPagePermissions(roleCode, roleName, paths) {
...
export function getRolePagePreset(roleCode) {
    const code = String(roleCode || '').trim().toUpperCase();
    return ROLE_PAGE_PRESETS[code] || null;
}
export function getRecommendedPagePermissions(roleCode) {
    return normalizePagePermissions(getRolePagePreset(roleCode)?.paths || []);
}
export function mergeRolePagePermissions(roleList) {
    return normalizePagePermissions(roleList.flatMap((item) => parseRoutePermissionsJson(item.routePermissionsJson)));
}
