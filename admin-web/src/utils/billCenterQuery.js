...
export function flattenBillCenterRouteQuery(query) {
    const flattened = {};
...
export function parseBillCenterRouteState(query, options) {
    ...
    const month = normalizeMonthText(query.billMonth);
    if (month) {
        state.month = month;
    }
    const keyword = firstQueryValue(query.billKeyword);
    if (keyword) {
        state.keyword = keyword;
    }
    const payMethod = firstQueryValue(query.billPayMethod).toUpperCase();
    if (payMethod) {
        state.payMethod = payMethod;
    }
    const pageNo = normalizePositiveInt(query.billPageNo, defaultPageNo);
    const pageSize = normalizePositiveInt(query.billPageSize, defaultPageSize);
    state.pageNo = pageNo;
    ...
    if (options?.lockScene) {
        ...
        const scene = normalizeScene(query.billScene);
        state.scene = scene || defaultScene;
    }
    return state;
}
export function buildBillCenterRouteQuery(state, routeQuery, options) {
    const query = {};
...
export function buildBillCenterRouteSignature(query) {
