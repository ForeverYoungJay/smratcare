...
export function normalizeLifecycleStage(flowStage, contractStatus) {
    const stage = normalizeUpper(flowStage);
    if (LIFECYCLE_STAGE_ORDER.includes(stage)) {
        return stage;
    }
...
export function lifecycleStageHint(stage) {
    const normalized = normalizeLifecycleStage(stage);
    if (normalized === 'PENDING_ASSESSMENT')
        return '建议先完成评估并归档报告，避免入住流程阻塞。';
    if (normalized === 'PENDING_BED_SELECT')
        return '当前可推进入住办理，完成选床后可进入待签署。';
    if (normalized === 'PENDING_SIGN')
        return '入住流程已到签署前置阶段，请尽快完成最终签署。';
    return '合同履约流程已闭环，可持续跟踪费用、服务与风险状态。';
}
export function lifecycleStageColor(stage) {
    const normalized = normalizeLifecycleStage(stage);
    if (normalized === 'PENDING_ASSESSMENT')
        return 'gold';
    if (normalized === 'PENDING_BED_SELECT')
