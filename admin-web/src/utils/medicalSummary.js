export const MEDICAL_RISK_LEVEL_LABEL_MAP = {
    LOW: '低风险',
    MEDIUM: '中风险',
    HIGH: '高风险',
    CRITICAL: '紧急风险'
};
export const MEDICAL_RISK_LEVEL_COLOR_MAP = {
    LOW: 'green',
    MEDIUM: 'blue',
    HIGH: 'orange',
    CRITICAL: 'red'
};
export function resolveMedicalRiskLabel(level) {
    const key = String(level || 'LOW').toUpperCase();
    return MEDICAL_RISK_LEVEL_LABEL_MAP[key] || MEDICAL_RISK_LEVEL_LABEL_MAP.LOW;
}
export function resolveMedicalRiskColor(level) {
    const key = String(level || 'LOW').toUpperCase();
    return MEDICAL_RISK_LEVEL_COLOR_MAP[key] || 'default';
}
export function normalizeRiskIndex(value) {
    return Math.max(0, Math.min(100, Number(value || 0)));
}
export function formatPercent(value, digits = 2) {
    return Number(value || 0).toFixed(digits);
}
export function createMedicalWorkbenchSummaryDefaults() {
    return {
