...
import type { MedicalCareWorkbenchSummaryQuery } from '../types'

type NumericField = 'incidentWindowDays' | 'overdueHours' | 'topResidentLimit' | 'riskResidentLookbackDays'

const DATE_RE = /^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])$/
const NUMERIC_FIELDS: NumericField[] = ['incidentWindowDays', 'overdueHours', 'topResidentLimit', 'riskResidentLookbackDays']

export const MEDICAL_WORKBENCH_NUMERIC_LIMITS: Record<NumericField, { min: number; max: number }> = {
...
export const MEDICAL_WORKBENCH_QUERY_DEFAULTS: Required<Pick<
  MedicalCareWorkbenchSummaryQuery,
  'incidentWindowDays' | 'overdueHours' | 'topResidentLimit' | 'riskResidentLookbackDays'
>> = {
...
function normalizeNumeric(field: NumericField, value: unknown, fallback: number) {
...
export function normalizeMedicalWorkbenchQuery(
  query?: Partial<MedicalCareWorkbenchSummaryQuery> | null,
  fallback: Required<Pick<MedicalCareWorkbenchSummaryQuery, NumericField>> = MEDICAL_WORKBENCH_QUERY_DEFAULTS
): MedicalCareWorkbenchSummaryQuery {
  const normalized: MedicalCareWorkbenchSummaryQuery = {
    ...
    riskResidentLookbackDays: normalizeNumeric(
      'riskResidentLookbackDays',
      query?.riskResidentLookbackDays,
      fallback.riskResidentLookbackDays
    )
  }
...
export function parseMedicalWorkbenchQueryPatch(routeQuery: LocationQuery): MedicalCareWorkbenchSummaryQuery {
  const patch: MedicalCareWorkbenchSummaryQuery = {}
...
export function buildMedicalWorkbenchRouteQuery(
  query: Partial<MedicalCareWorkbenchSummaryQuery>,
  ...
  const result: Record<string, string> = {
    ...
    riskResidentLookbackDays: String(
      normalized.riskResidentLookbackDays ?? MEDICAL_WORKBENCH_QUERY_DEFAULTS.riskResidentLookbackDays
    )
  }
  if (normalized.date) {
