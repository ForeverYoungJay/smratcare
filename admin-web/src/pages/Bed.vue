      ...
      <div class="bed-grid">
        <div v-for="bed in bedAll" :key="bed.id" class="bed-card">
          <div class="bed-header">
            <span>{{ bed.bedNo }}</span>
            <a-tag :color="bed.elderId ? 'blue' : 'green'">{{ bed.elderId ? '已入住' : '空闲' }}</a-tag>
          </div>
          <div class="bed-meta">房间 {{ bed.roomId }}</div>
...
import type { RoomItem, BedItem, ApiResult, PageResult } from '../types/api'
...
async function load() {
  ...
  try {
    const roomRes: ApiResult<PageResult<RoomItem>> = await getRoomPage(roomQuery)
    const bedRes: ApiResult<PageResult<BedItem>> = await getBedPage(bedQuery)
    const roomListRes: ApiResult<RoomItem[]> = await getRoomList()
    const bedListRes: ApiResult<BedItem[]> = await getBedList()
    if (roomRes.code === 0) {
      rooms.value = roomRes.data.records
      roomPagination.total = roomRes.data.total || roomRes.data.records.length
    }
    if (bedRes.code === 0) {
      beds.value = bedRes.data.records
      bedPagination.total = bedRes.data.total || bedRes.data.records.length
    }
    if (roomListRes.code === 0) roomAll.value = roomListRes.data
    if (bedListRes.code === 0) bedAll.value = bedListRes.data
  } catch {
    rooms.value = [
      { id: 10, roomNo: 'A101' },
      { id: 11, roomNo: 'A102' }
