...
import type { ElderItem, PageResult, ApiResult, BedItem } from '../types/api'
...
async function load() {
  loading.value = true
  try {
    const res: ApiResult<PageResult<ElderItem>> = await getElderPage(query)
    if (res.code === 0) {
    ...
    const bedRes: ApiResult<BedItem[]> = await getBedList()
    if (bedRes.code === 0) beds.value = bedRes.data
  } catch {
    list.value = [
