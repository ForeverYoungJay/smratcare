  ...
  <PageContainer title="月账单" subTitle="账单查询与支付登记">
    <SearchForm :model="query" @search="fetchData" @reset="onReset">
      ...
      <a-form-item label="老人ID">
        <a-input v-model:value="query.elderId" placeholder="老人ID" />
      </a-form-item>
    </SearchForm>
...
import type { BillItem, ApiResult, PageResult } from '../types/api'

const query = reactive({ month: undefined as string | undefined, elderId: undefined as string | undefined, pageNo: 1, pageSize: 10 })
...
async function fetchData() {
  loading.value = true
  try {
    const res: ApiResult<PageResult<BillItem>> = await getBillPage(query)
    if (res.code === 0) {
