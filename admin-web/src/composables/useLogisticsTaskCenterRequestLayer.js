...
export function buildTaskCenterSummaryRequest(summaryQuery) {
    if (Object.keys(summaryQuery || {}).length === 0) {
        return { lite: true };
    }
    return { ...normalizeLogisticsWorkbenchQuery(summaryQuery), lite: true };
}
export async function requestTaskCenterSnapshot(summaryQuery) {
...
export function useLogisticsTaskCenterRequestLayer(summaryQuery) {
    const resolvedSummaryQuery = computed(() => normalizeLogisticsWorkbenchQuery(summaryQuery.value));
    const summaryWindowHint = computed(() => `窗口 ${resolvedSummaryQuery.value.windowDays} 天 / 临期 ${resolvedSummaryQuery.value.expiryDays} 天 / 维修超时 ${resolvedSummaryQuery.value.overdueDays} 天 / 维保 ${resolvedSummaryQuery.value.maintenanceDueDays} 天`);
