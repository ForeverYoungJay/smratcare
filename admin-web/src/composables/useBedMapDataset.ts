...
import type { BaseConfigItem, BedItem, RoomItem } from '../types/api'

const plainBeds = ref<BedItem[]>([])
const riskBeds = ref<BedItem[]>([])
...
let plainBedPromise: Promise<BedItem[]> | null = null
let riskBedPromise: Promise<BedItem[]> | null = null
let roomListPromise: Promise<RoomItem[]> | null = null
let configPromise: Promise<void> | null = null

async function ensureBedMapLoaded(includeRisk = false, force = false) {
  if (includeRisk) {
    if (!force && riskBeds.value.length) return riskBeds.value
    if (!force && riskBedPromise) return riskBedPromise
  ...
  if (!force && plainBeds.value.length) return plainBeds.value
  if (!force && plainBedPromise) return plainBedPromise
...
async function ensureRoomListLoaded(force = false) {
  if (!force && roomList.value.length) return roomList.value
  if (!force && roomListPromise) return roomListPromise
...
async function ensureResidenceConfigLoaded(force = false) {
  if (!force && roomTypeItems.value.length && bedTypeItems.value.length && areaItems.value.length) return
...
async function refreshBedMapDataset(options?: {
  includeRisk?: boolean
  rooms?: boolean
  residenceConfig?: boolean
}) {
  const tasks: Array<Promise<any>> = []
  tasks.push(ensureBedMapLoaded(Boolean(options?.includeRisk), true))
  if (options?.rooms) tasks.push(ensureRoomListLoaded(true))
  if (options?.residenceConfig) tasks.push(ensureResidenceConfigLoaded(true))
