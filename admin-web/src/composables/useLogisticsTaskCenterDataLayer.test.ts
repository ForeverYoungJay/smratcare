...
import {
  retainTaskCenterSelectionKeys,
  useLogisticsTaskCenterDataLayer
} from './useLogisticsTaskCenterDataLayer'
import type { LogisticsTaskCenterDataSnapshot } from './useLogisticsTaskCenterRequestLayer'

function createSnapshot(): LogisticsTaskCenterDataSnapshot {
  return {
    summary: {
      lowStockCount: 0,
      expiringCount: 0,
    ...
    cleaningRows: [{ id: 1 } as any, { id: 2 } as any],
    maintenanceRows: [{ id: 10 } as any, { id: 12 } as any],
    deliveryRows: [{ id: 20 } as any],
    adjustmentRows: [{ id: 30 } as any]
  }
}
...
describe('useLogisticsTaskCenterDataLayer', () => {
  ...
  it('loads snapshot and preserves valid selections', async () => {
    ...
    const cleaningRows = ref<any[]>([])
    const maintenanceRows = ref<any[]>([])
    const deliveryRows = ref<any[]>([])
    const adjustmentRows = ref<any[]>([])
    const selectedCleaningKeys = ref<Array<number | string>>([2, 99])
    const selectedMaintenanceKeys = ref<Array<number | string>>([10, 98])
    const selectedDeliveryKeys = ref<Array<number | string>>([20, 200])
  ...
  it('clears selections and reports error when request fails', async () => {
    ...
    const layer = useLogisticsTaskCenterDataLayer({
      ...
      lastLoadedAt: ref(''),
      summary: ref<any>(undefined),
      cleaningRows: ref<any[]>([]),
      maintenanceRows: ref<any[]>([]),
      deliveryRows: ref<any[]>([]),
      adjustmentRows: ref<any[]>([]),
      selectedCleaningKeys: ref<Array<number | string>>([1]),
      selectedMaintenanceKeys: ref<Array<number | string>>([2]),
      selectedDeliveryKeys: ref<Array<number | string>>([3]),
      onError
    })
